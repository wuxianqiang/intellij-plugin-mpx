...
import com.hxz.mpxjs.codeInsight.tags.VueInsertHandler
import com.hxz.mpxjs.editor.VueComponentSourceEdit
import com.hxz.mpxjs.index.VueFileVisitor
...
class VueExtractComponentDataBuilder(private val list: List<XmlTag>) {
  ...
  fun createNewComponent(newComponentName: String): VirtualFile? {
    ...
    val virtualFile = folder.virtualFile.createChildData(this, toAsset(newComponentName, true) + ".mpx")
...
export default {
  ...
  private fun psiOperationOnText(text: String, operation: (PsiFile) -> Unit): String {
    val dummyFile = PsiFileFactory.getInstance(containingFile.project).createFileFromText("a.mpx", VueFileType.INSTANCE, text)
    operation(dummyFile)
  ...
  fun replaceWithNewTag(replaceName: String): PsiElement {
    ...
    val dummyFile = PsiFileFactory.getInstance(project).createFileFromText("dummy.mpx",
